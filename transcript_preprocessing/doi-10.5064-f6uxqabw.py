...
in_directory = "data/raw_data/doi-10.5064-f6uxqabw"
out_individual = "data/processed_transcripts/doi-10.5064-f6uxqabw"
out_combined = "data/combined_transcripts/doi-10.5064-f6uxqabw__combined_transcripts.csv"
...
def process_pdf(input_pdf, output_csv):
    ...
    pattern = r"(Interviewer|Interviewee):\s*(.*?)(?=(Interviewer|Interviewee|$))"
    matches = re.findall(pattern, transcript, flags=re.S)

    rows = []
    for match in matches:
        speaker = match[0].strip().replace('Interviewer', 'interviewer').replace('Interviewee', 'participant')
...
def collect_input_pdf_filenames(directory):
    ...
    for root, dirs, files in os.walk(directory):
        for file in files:
            if "Interview_CG" in file or "Interview_Patient" in file:
