...
in_directory = "data/raw_data/doi-10.5064-f6xzv8bz"
out_individual = "data/processed_transcripts/doi-10.5064-f6xzv8bz"
out_combined = "data/combined_transcripts/doi-10.5064-f6xzv8bz__combined_transcripts.csv"
...
def process_pdf(input_pdf, output_csv):
    ...
    # Remove timestamp segments like [X:XX:XX.X] (or similar variants) before parsing
    transcript = re.sub(r"\[\d+:\d{2}:\d{2}\.\d+\]", "", transcript)

    # Extract utterances where tags start on a new line, begin with a capital letter, and end with ':'
    # Capture the tag (before ':') and the following text up to the next similarly-formatted tag or end
    pattern = r"([A-Z][^\n:]*):\s*(.*?)(?=(?:[A-Z][^\n:]*:|$))"
    ...
    for match in matches:
        tag = match[0].strip()
        speaker = 'interviewer' if tag == 'Interviewer' else 'participant'
