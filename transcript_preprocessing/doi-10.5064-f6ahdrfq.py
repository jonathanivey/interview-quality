...
in_directory = "data/raw_data/doi-10.5064-f6ahdrfq"
out_individual = "data/processed_transcripts/doi-10.5064-f6ahdrfq"
out_combined = "data/combined_transcripts/doi-10.5064-f6ahdrfq.csv"
...
def process_pdf(input_pdf, output_csv):
    ...
    with pdfplumber.open(input_pdf) as pdf:
        for page in pdf.pages:
            ...
            bbox = (0, 80, width, height - 60)
            text = page.within_bbox(bbox).extract_text()
            if text:
                transcript += text

    # Extract interviewer and various interviewee utterances with regex
    pattern = r"(INTERVIEWER|MARINE \d+|TEAM MEMBER \d+|LEADERSHIP \d+):\s*(.*?)(?=(INTERVIEWER|MARINE \d+|TEAM MEMBER \d+|LEADERSHIP \d+|$))"
    matches = re.findall(pattern, transcript, flags=re.S)

    rows = []
    for match in matches:
        # Convert any interviewee format to 'participant', keep interviewer as is
...
def collect_input_pdf_filenames(directory):
    ...
    for root, dirs, files in os.walk(directory):
        for file in files:
            if "TEAMMEM" in file or "LEADER" in file or "MARINE" in file:
    ...
    for root, dirs, files in os.walk(directory):
        for file in files:
            if file.endswith('.csv'):
                ...
                participant_name = source_name.split('SNCO-')[-1]  # Extract participant identifier
