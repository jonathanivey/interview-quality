...
in_directory = "data/raw_data/doi-10.5064-f6qhvgui/InterviewTranscripts"
out_individual = "data/processed_transcripts/doi-10.5064-f6qhvgui"
out_combined = "data/combined_transcripts/doi-10.5064-f6qhvgui__combined_transcripts.csv"
...
def process_pdf(input_pdf, output_csv):
    ...
    with pdfplumber.open(input_pdf) as pdf:
        for page in pdf.pages:
            ...
            bbox = (0, 0, width, height-65)
            text = page.within_bbox(bbox).extract_text()
            if text:
                transcript += text

    # Extract interviewer and interviewee utterances
    # Match tags that start with 'Interviewer' or 'Interviewee', may contain
    # any characters, and end with either ']' or ':' (e.g., 'Interviewer 1:',
    # '[Interviewee]', 'Interviewee (participant):').
    tag_pat = r"(?:Interviewer[^\]\:]*[\]\:]|Interviewee[^\]\:]*[\]\:])"
    pattern = rf"({tag_pat})\s*(.*?)(?=(?:{tag_pat})|$)"
    matches = re.finditer(pattern, transcript, flags=re.S | re.I)

    rows = []
    for m in matches:
        speaker_label = m.group(1).strip()
        # normalize label by removing trailing ']' or ':'
        normalized = re.sub(r"[\]\:]+$", "", speaker_label).strip()
        speaker = 'interviewer' if normalized.lower().startswith('interviewer') else 'participant'
        text = m.group(2).strip().replace('\n', ' ')
        # Remove leading timestamp of form XX:XX] (allow optional leading '[')
        text = re.sub(r'^\[?[0-9]{2}:[0-9]{2}\]\s*', '', text)
...
def collect_input_pdf_filenames(directory):
    ...
    for root, dirs, files in os.walk(directory):
        for file in files:
            if "Interview_" in file and "_30_" not in file and "_20_" not in file and "_27_" not in file:
