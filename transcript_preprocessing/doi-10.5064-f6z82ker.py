...
in_directory = "data/raw_data/doi-10.5064-f6z82ker"
out_individual = "data/processed_transcripts/doi-10.5064-f6z82ker"
out_combined = "data/combined_transcripts/doi-10.5064-f6z82ker__combined_transcripts.csv"
...
def process_pdf(input_pdf, output_csv):
    ...
    # Handle tag formats exactly: [P], [I], [P]:, [I]:, P:, I:
    # Do NOT match 'R' or standalone 'I'/'P' without bracket or colon.
    tag_regex = re.compile(r"(?:\[\s*([IP])\s*\]\s*:?)|(?:\b([IP]):)", flags=re.I)
    matches = list(tag_regex.finditer(transcript))

    rows = []
    for idx, m in enumerate(matches):
        letter = (m.group(1) or m.group(2) or '').upper()
        speaker = 'interviewer' if letter == 'I' else 'participant'
        start = m.end()
        end = matches[idx + 1].start() if idx + 1 < len(matches) else len(transcript)
        text = transcript[start:end].strip().replace('\n', ' ')
        if text:
...
def collect_input_pdf_filenames(directory):
    ...
    for root, dirs, files in os.walk(directory):
        for file in files:
            if "Interview" in file and "Guide" not in file:
