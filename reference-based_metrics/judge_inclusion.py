...
def build_prompt(result: str, excerpt: str, previous_excerpt: str = None, context_blurb: str = None) -> str:
    if previous_excerpt:
...
Your task is to rate the likelihood that the participant statement from the current interview excerpt below contributed to the provided results section on a scale from 1-5. In addition to the current excerpt, you are also provided with a short context blurb and the interview excerpt that immediately preceded the current excerpt in the transcript. These two sections are only to understand the context of the current excerpt, and your rating should be for participant statement in the current excerpt.

Scoring Rubric:
1. The statement is unrelated to the results section or contradicts it.
2. Tangential relation; discusses the topic but offers no specific substance.
3. Aligns with the results section but is general or vague.
4. Provides an example or sentiment that matches the results section’s conclusion.
5. Appears in the results section and likely served as a primary source for it.
...
CRITICAL: Output only a single digit (1, 2, 3, 4, or 5). Do not write any additional text.

"""


def judge_row(result: str, excerpt: str, model: str = "gpt-5.4", max_retries: int = 1) -> str:
    prompt = build_prompt(result, excerpt)
    attempt = 0
...
def process_all(input_dir: Path, results_dir: Path, output_dir: Path, model: str = "gpt-5.4", delay: float = 0.05, max_rows: int = None, only_csv: str = None, dry_run: bool = False) -> None:
    ...
    for csv_file in csv_files:
        stem = csv_file.stem
        result_file = _resolve_companion_file(results_dir, stem, ".txt")
        if not result_file.exists():
            print(f"Skipping {csv_file.name}: no matching result file {result_file}", file=sys.stderr)
            continue

        print(f"Processing {csv_file.name} -> {result_file.name}")
        ...
        # Read results sections
        raw = result_file.read_text(encoding="utf-8")
        sections = [s.strip() for s in raw.split('%&%') if s.strip()]
        if not sections:
            print(f"No result sections found in {result_file}", file=sys.stderr)
            continue
        ...
        # prepare output columns for each section
        section_cols = [f"gpt5_judgement_{i+1}" for i in range(len(sections))]
        out_fieldnames = fieldnames + section_cols

        out_path = output_dir / csv_file.name
        # Build batch request lines for all (row, section) pairs
        request_lines = []
        for idx, row in enumerate(rows):
            ...
            for si, section in enumerate(sections):
                prompt = build_prompt(section, excerpt, previous_excerpt=previous_excerpt, context_blurb=context_blurb)
                custom_id = f"{stem}:{idx}:{si}"
        ...
        with out_path.open("w", encoding="utf-8", newline="") as outf:
            ...
            for idx, row in enumerate(rows):
                for si, _ in enumerate(sections):
                    colname = section_cols[si]
                    custom_id = f"{stem}:{idx}:{si}"
...
def main():
    parser = argparse.ArgumentParser(description="Batch LLM judge for paired transcripts vs. results sections")
    parser.add_argument("--input-dir", default="data/excerpt_transcripts", help="Directory with input CSVs")
    parser.add_argument("--results-dir", default="data/results", help="Directory with results .txt files")
    parser.add_argument("--output-dir", default="data/reference-based_metrics/inclusion", help="Directory to write outputs")
    ...
    process_all(input_dir, results_dir, output_dir, model=args.model, delay=args.delay, max_rows=args.max_rows, only_csv=args.only_csv, dry_run=args.dry_run)
