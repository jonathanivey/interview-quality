...
def build_prompt(current_excerpt: str, research_questions: str, previous_excerpt: str = None, include_previous: bool = False, context_blurb: str = None) -> str:
    if previous_excerpt:
...
Your task is to analyze the interviewer statement in the current interview excerpt below and determine which of the following categories it fits in based on the strategy that the interviewer is using. In addition to the current excerpt, you are also provided with a short context blurb and the interview excerpt that immediately preceded the current excerpt in the transcript. These two sections are only to understand the context of the current excerpt, and your rating should be for interviewer statement in the current excerpt.

Possible Categories:
1. Introductory/Contextualization Questions: Open-ended questions that may be unrelated to the overall research questions but are designed to give an understanding of the participant or context.
2. Support and Rapport Building: A statement designed to make a connection with the participant, provide support, or let the participant know that the purpose of the interview is being fulfilled. 
3. Follow-up / Elaboration Probe: A statement designed to encourage a participant to continue talking. It may be a simple “uh-huh” or “mmm,” or it could also be a direct call such as, “Could you say some more about that?”
4. Specifying / Detail-Oriented Probe: Questions that follow up to ask who, where, what, when, or how to obtain a complete and detailed picture of an activity or experience.
5. Direct Questioning: A question that directly introduces topics or dimensions and asks the respondent about them.
6. Indirect / Projective Questioning: Indirect questions that may ask about the attitudes of others or encourage an indirect statement of the participant’s own motivations, attitudes, or emotions.
7. Structuring: A statement that controls the structure of the interview by transitioning topics, redirecting respondents, or breaking off participant answers that may be irrelevant to the purpose of the interview.
8. Interpreting: A statement that rephrases or interprets answers provided by the participant to get clarification or reach common ground with the participant.
...
CRITICAL: Output only digits (1, 2, 3, 4, 5, 6, 7, 8) separated by commas. Do not write any additional text.

"""

def judge_row(excerpt: str, research_questions: str = "", model: str = "gpt-5.4", max_retries: int = 1) -> str:
    prompt = build_prompt(excerpt, research_questions)
    attempt = 0
    ...
    for fid in output_ids:
        try:
            ...
            for line in text.splitlines():
                ...
                digits = re.findall(r"[1-9]", out_text)
                # preserve order and dedupe
                seen = set()
                out = []
                for d in digits:
                    if d not in seen:
                        seen.add(d)
                        out.append(d)
                results_by_custom_id[custom_id] = ",".join(out) if out else ""
        except Exception as e:
...
def process_all(input_dir: Path, output_dir: Path, model: str = "gpt-5.4", delay: float = 0.05, max_rows: int = None, only_csv: str = None, dry_run: bool = False) -> None:
    ...
    for csv_file in csv_files:
        ...
        # prepare eight output columns for each category (1..9)
        section_cols = [f"gpt5_cat_{i}" for i in range(1, 10)]
        out_fieldnames = fieldnames + section_cols

        out_path = output_dir / csv_file.name
        # Read research questions file corresponding to this CSV (by stem)
        rqs_path = _resolve_companion_file(Path("data/research_questions"), stem, ".txt")
        if rqs_path.exists():
            try:
                research_questions = rqs_path.read_text(encoding="utf-8")
            except Exception:
                research_questions = ""
                print(f"Warning: failed to read {rqs_path}", file=sys.stderr)
        else:
            research_questions = ""
            print(f"Warning: research questions file not found: {rqs_path}", file=sys.stderr)

        # Build batch request lines: one request per row (excerpt only)
        # Detect source column so we can include the previous excerpt
        source_col = None
        for f in fieldnames:
            if f and f.lower() == "source":
                source_col = f
                break
        ...
        for idx, row in enumerate(rows):
            ...
            include_prev = False
            prev_excerpt = None
            if idx > 0 and source_col:
                prev_row = rows[idx - 1]
                cur_source = (row.get(source_col) or "").strip()
                prev_source = (prev_row.get(source_col) or "").strip()
                if cur_source and prev_source and cur_source == prev_source:
                    prev_val = (prev_row.get(excerpt_col) or "").strip()
                    if prev_val:
                        include_prev = True
                        prev_excerpt = prev_val

            prompt = build_prompt(excerpt, research_questions, previous_excerpt=prev_excerpt, include_previous=include_prev, context_blurb=context_blurb)
        ...
        with out_path.open("w", encoding="utf-8", newline="") as outf:
            ...
            for idx, row in enumerate(rows):
                custom_id = f"{stem}:{idx}"
                digits_str = results_by_custom_id.get(custom_id, "")
                found = re.findall(r"[1-9]", digits_str)
                for i in range(1, 10):
                    col = f"gpt5_cat_{i}"
                    row[col] = "1" if str(i) in found else "0"
                writer.writerow(row)
...
def main():
    ...
    parser.add_argument("--output-dir", default="data/interviewer_metrics/interviewer_techniques", help="Directory to write outputs")
