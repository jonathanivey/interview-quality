...
Rate how relevant the participant's statement is to the specific question asked by the interviewer in the current interview excerpt below on a scale from 1 to 3. In addition to the current excerpt, you are also provided with a short context blurb and the interview excerpt that immediately preceded the current excerpt in the transcript. These two sections are only to understand the context of the current excerpt, and your rating should be for participant statement in the current excerpt.

Scoring Rubric:
1. The participant statement is completely unrelated to the question asked, avoids the question entirely, or addresses a totally different topic.
2. The participant statement is related to the general topic of the question but drifts or answers a different question than the one posed.
3. The participant statement directly answers the specific question posed by the interviewer.
...
def main():
    ...
    parser.add_argument("--output-dir", default="data/reference-free_metrics/immediate_relevance", help="Directory to write outputs")
