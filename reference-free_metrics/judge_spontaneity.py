...
Rate the spontaneity of the participant statement from in the current interview excerpt below on a scale from 1 to 3. In addition to the current excerpt, you are also provided with a short context blurb and the interview excerpt that immediately preceded the current excerpt in the transcript. These two sections are only to understand the context of the current excerpt, and your rating should be for participant statement in the current excerpt.

Scoring Rubric:
1. The participant statement only confirms or reiterates information provided in the interviewer’s statement.
2. The participant statement adds additional information beyond what was provided in the interviewer’s statement but remains within the topic posed.
3. The participant statement introduces a new topic that may be related but was not introduced in the interviewer’s statement.
...
def main():
    ...
    parser.add_argument("--output-dir", default="data/reference-free_metrics/spontaneity", help="Directory to write outputs")
