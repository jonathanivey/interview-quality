...
def build_prompt(research_question: str, excerpt: str, previous_excerpt: str = None, context_blurb: str = None) -> str:
    if previous_excerpt:
...
def judge_row(research_question: str, excerpt: str, model: str = "gpt-5.4", max_retries: int = 1) -> str:
    prompt = build_prompt(research_question, excerpt)
    attempt = 0
...
def process_all(input_dir: Path, rq_dir: Path, output_dir: Path, model: str = "gpt-5.4", delay: float = 0.05, max_rows: int = None, only_csv: str = None, dry_run: bool = False) -> None:
    ...
    for csv_file in csv_files:
        stem = csv_file.stem
        rq_file = _resolve_companion_file(rq_dir, stem, ".txt")
        if not rq_file.exists():
            print(f"Skipping {csv_file.name}: no matching research question file {rq_file}", file=sys.stderr)
            continue

        print(f"Processing {csv_file.name} -> {rq_file.name}")
        ...
        # Read research question sections
        raw = rq_file.read_text(encoding="utf-8")
        sections = [s.strip() for s in raw.split('\n') if s.strip()]
        if not sections:
            print(f"No research questions found in {rq_file}", file=sys.stderr)
            continue
...
def main():
    parser = argparse.ArgumentParser(description="Batch LLM judge for paired transcripts vs. research questions")
    parser.add_argument("--input-dir", default="data/excerpt_transcripts", help="Directory with input CSVs")
    parser.add_argument("--rq-dir", default="data/research_questions", help="Directory with research question .txt files")
    parser.add_argument("--output-dir", default="data/reference-free_metrics/rq_relevance", help="Directory to write outputs")
    ...
    if not rq_dir.exists():
        print(f"Research question directory not found: {rq_dir}", file=sys.stderr)
        sys.exit(2)

    process_all(input_dir, rq_dir, output_dir, model=args.model, delay=args.delay, max_rows=args.max_rows, only_csv=args.only_csv, dry_run=args.dry_run)
